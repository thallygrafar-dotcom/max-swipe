...
export default function SwipeMaxInvisibleModal({ vslId, onClose }: Props) {
  ...
  if (loading) {
    return (
      ...
      >
        <div
          className="relative flex h-[88vh] w-full max-w-[1180px] items-center justify-center overflow-hidden rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(22,22,28,0.98)_0%,rgba(13,13,18,1)_100%)] shadow-[0_30px_100px_rgba(0,0,0,0.45)]"
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute right-6 top-6 z-50 flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black/40 text-zinc-300 backdrop-blur-md transition-all hover:bg-black/60"
          >
            <X className="h-4 w-4" />
          </button>
      ...
      >
        ...
        <div className="relative z-10 flex w-full flex-col lg:flex-row">
          ...
          <main className="min-h-0 flex-1 overflow-hidden">
            ...
            >
              {!normalizedAnalysis && <EmptyState onClose={onClose} />}
